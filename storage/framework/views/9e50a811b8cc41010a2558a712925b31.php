...
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
    <div>
        <h1 style="margin:0 0 4px;"><?php echo e($poll->exists ? 'Poll bewerken' : 'Nieuwe poll'); ?></h1>
        <p class="meta" style="margin:0;"><?php echo e($poll->exists ? 'Wijzig de gegevens van deze poll.' : 'Vul de gegevens in en voeg vragen toe.'); ?></p>
    </div>
    <a class="btn" href="<?php echo e(route('admin.polls.index')); ?>">← Terug</a>
</div>
...
<form method="post" action="<?php echo e($action); ?>">
    ...
    <div class="card" style="margin-bottom:16px;">
        ...
        <div class="field">
            ...
            <input id="title" name="title" placeholder="Geef de poll een duidelijke naam" value="<?php echo e(old('title', $poll->title)); ?>" required>
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="field">
            ...
            <textarea id="question" name="question" rows="3" placeholder="Voeg hier een beschrijving of toelichting in..."><?php echo e(old('question', $poll->question ?? '')); ?></textarea>
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="field" style="max-width:280px;">
            ...
            <select id="status" name="status" required>
                <option value="active"   <?php if(old('status', $poll->exists ? $poll->status : 'active') === 'active'): echo 'selected'; endif; ?>>Publiek</option>
                <option value="archived" <?php if(old('status', $poll->status) === 'archived'): echo 'selected'; endif; ?>>Archief</option>
            </select>
        </div>
        ...
        <div class="grid grid-2">
            <div class="field">
                ...
                <input id="opens_at" type="datetime-local" name="opens_at" value="<?php echo e(old('opens_at', optional($poll->opens_at)->format('Y-m-d\TH:i'))); ?>">
            </div>
            <div class="field">
                <label for="closes_at">Sluit op <span style="font-weight:400; color:var(--muted);">(optioneel)</span></label>
                <input id="closes_at" type="datetime-local" name="closes_at" value="<?php echo e(old('closes_at', optional($poll->closes_at)->format('Y-m-d\TH:i'))); ?>">
            </div>
        </div>
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="field" style="max-width:280px;">
            ...
            <input id="access_code" type="text" name="access_code" placeholder="Laat leeg voor openbare poll" value="<?php echo e(old('access_code', $poll->access_code ?? '')); ?>" style="text-transform:uppercase;">
    ...
    <div class="card" style="margin-bottom:16px; background:#fffbeb; border-color:#fcd34d;">
        <p style="margin:0;">Vragen beheer je op de <a href="<?php echo e(route('admin.polls.show', $poll)); ?>" style="color:var(--accent); font-weight:600;">detailpagina van de poll</a>.</p>
    </div>
    <?php endif; ?>
    ...
    <div style="display:flex; gap:10px;">
        <button class="btn btn-primary" type="submit" style="padding:10px 24px;">
            <?php echo e($poll->exists ? 'Wijzigingen opslaan' : 'Poll aanmaken'); ?>

        </button>
        <a class="btn" href="<?php echo e(route('admin.polls.index')); ?>" style="padding:10px 24px;">Annuleren</a>
    </div>
</form>
...
<?php if(!$poll->exists): ?>
<script>
const typeLabels    = <?php echo json_encode($typeLabels ?? [], 15, 512) ?>;
const defaultsByType = <?php echo json_encode($defaultsByType ?? [], 15, 512) ?>;
const ratingType    = '<?php echo e(\App\Support\PollType::RATING_STARS); ?>';
const openTextType  = '<?php echo e(\App\Support\PollType::OPEN_TEXT); ?>';
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Elementa-polls\Polls-Elementa\resources\views\admin\polls\form.blade.php ENDPATH**/ ?>